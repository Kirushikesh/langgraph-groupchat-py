...
lint format: PYTHON_FILES=src/langgraph_groupchat/ tests/
lint_diff format_diff: PYTHON_FILES=$(shell git diff --relative=src/langgraph_groupchat/ tests/ --name-only --diff-filter=d master | grep -E '\.py$$|\.ipynb$$')
